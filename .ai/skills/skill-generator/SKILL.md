...
2. LEVEL 1 — Run automated scan:
   python3 ~/.ai/skills/skill-generator/scripts/security-scan.py <skill-path>
3. Check exit code:
...
2. **LEVEL 1** — Run automated scanner:
   ```bash
   python3 ~/.ai/skills/skill-generator/scripts/security-scan.py <path>
   ```
...
```bash
python3 ~/.ai/skills/skill-generator/scripts/security-scan.py ./skill-name/
```
...
```bash
python ~/.ai/skills/dependency-graph/scripts/visualize.py $ARGUMENTS
```
```
...
1. **Local**: Keep in `~/.ai/skills/` for personal use
2. **Project**: Add to `.ai/skills/` and commit
